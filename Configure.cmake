...
if (NOT CMAKE_CROSSCOMPILING AND NOT SLEEF_FORCE_FIND_PACKAGE_SSL)
  ...
  include_directories(${PATH_OPENSSL}/include)
  link_directories(${PATH_OPENSSL}/lib) 
  find_library(LIBSSL ssl ${PATH_OPENSSL}/lib)
  find_library(LIBCRYPTO crypto ${PATH_OPENSSL}/lib)
  message("${LIBSSL}, ${LIBCRYPTO}")
...
if (NOT (RUNNING_ON_APPVEYOR AND SLEEF_CLANG_ON_WINDOWS))
  ...
  include_directories(${PATH_MPFR}/include)
  link_directories(${PATH_MPFR}/lib) 
  include_directories(${PATH_GMP}/include)
  link_directories(${PATH_GMP}/lib) 
  find_library(LIB_MPFR mpfr ${PATH_MPFR}/lib)
  find_library(LIBM m)
  find_library(LIBGMP gmp ${PATH_GMP}/lib)
...
set(SLEEF_SUPPORTED_GNUABI_EXTENSIONS 
	SSE2 AVX AVX2 AVX512F ADVSIMD SVE KVX_SIMD KVX_BIG_SIMD
...
elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "kvx")
  set(SLEEF_ARCH_KVX ON CACHE INTERNAL "True for kvx architecture")
  set(COMPILER_SUPPORTS_BUILTIN_MATH 0)
  set(LIBM "-lm")

  # This is used to pass parameters to the mkrename utilitary.
  # argument list is: deterministic form, dp vector size, sp vector size, dp type name, sp type name, int vector name, nt vector name, arch macro, function suffix
  set(HEADER_PARAMS_KVX_SIMD_     finz_ 2 4 "float64x2_t" "float32x4_t" "int32x2_t" "int32x4_t" __KVX__)
  set(HEADER_PARAMS_KVX_SIMD      finz_ 2 4 "float64x2_t" "float32x4_t" "int32x2_t" "int32x4_t" __KVX__ kvx)
  set(HEADER_PARAMS_KVX_BIG_SIMD_     finz_ 4 8 "float64x4_t" "float32x8_t" "int32x4_t" "int32x8_t" __KVX__)
  set(HEADER_PARAMS_KVX_BIG_SIMD      finz_ 4 8 "float64x4_t" "float32x8_t" "int32x4_t" "int32x8_t" __KVX__ kvx) 
  
  # this is used to alias functions
  set(ALIAS_PARAMS_KVX_SIMD_DP  2 "\'double" "__attribute__((vector_size(sizeof(double)*2)));\'" "\'int" "__attribute__((vector_size(sizeof(int)*4)));\'" d kvx)
  set(ALIAS_PARAMS_KVX_SIMD_SP -4 "\'float"  "__attribute__((vector_size(sizeof(float)*4)));\'"  "\'int" "__attribute__((vector_size(sizeof(int)*4)));\'" d kvx)
  set(ALIAS_PARAMS_KVX_BIG_SIMD_DP  4 "\'double" "__attribute__((vector_size(sizeof(double)*4)));\'" "\'int" "__attribute__((vector_size(sizeof(int)*8)));\'" d kvx)
  set(ALIAS_PARAMS_KVX_BIG_SIMD_SP -8 "\'float"  "__attribute__((vector_size(sizeof(float)*8)));\'"  "\'int" "__attribute__((vector_size(sizeof(int)*8)));\'" d kvx)

  set(TESTER3_DEFINITIONS_KVX_SIMD      ATR=finz_ DPTYPE=float64x2_t SPTYPE=float32x4_t DPTYPESPEC=d2 SPTYPESPEC=f4 EXTSPEC=kvx)
  set(TESTER3_DEFINITIONS_KVX_BIG_SIMD  ATR=finz_ DPTYPE=float64x4_t SPTYPE=float32x8_t DPTYPESPEC=d4 SPTYPESPEC=f8 EXTSPEC=kvx)
  set(COMPILER_SUPPORTS_KVX_SIMD 1)
  set(COMPILER_SUPPORTS_KVX_BIG_SIMD 1)
...
command_arguments(RENAME_PARAMS_KVX_SIMD        finz_ 2 4 kvx)
command_arguments(RENAME_PARAMS_KVX_BIG_SIMD    finz_ 4 8 kvx)

# third argument is ISA field in GNU mangling
command_arguments(RENAME_PARAMS_GNUABI_KVX_SIMD          kvx b 2 4 double float int int __KVX__)
command_arguments(RENAME_PARAMS_GNUABI_KVX_BIG_SIMD      kvx b 4 8 double float int int __KVX__)
command_arguments(MKMASKED_PARAMS_GNUABI_KVX_SIMD_dp     kvx b 2)
command_arguments(MKMASKED_PARAMS_GNUABI_KVX_SIMD_sp     kvx b -4)
command_arguments(MKMASKED_PARAMS_GNUABI_KVX_BIG_SIMD_dp kvx b 4)
command_arguments(MKMASKED_PARAMS_GNUABI_KVX_BIG_SIMD_sp kvx b -8)
...
if(CMAKE_C_COMPILER_ID MATCHES "(GNU|Clang)")
  # Always compile sleef with -ffp-contract=off.
  set(FLAGS_STRICTMATH "-ffp-contract=off")

  # Without the options below, gcc generates calls to libm
  string(CONCAT FLAGS_OTHERS "-flax-vector-conversions --save-temps -fPIC -fno-math-errno -fno-trapping-math")
