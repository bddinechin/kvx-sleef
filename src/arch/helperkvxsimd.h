...
#define VATTRIBUTE(type, length) __attribute__((ext_vector_type(length)))

typedef uint32_t vmask VATTRIBUTE(uint32_t, VECTLENDP*2);
typedef uint32_t vopmask VATTRIBUTE(uint32_t, VECTLENDP*2);

typedef double vdouble VATTRIBUTE(double, VECTLENDP);
typedef int32_t vint VATTRIBUTE(int32_t, VECTLENDP);

typedef float vfloat VATTRIBUTE(float, VECTLENSP);
typedef int32_t vint2 VATTRIBUTE(int32_t, VECTLENSP);
...
#define VATTRIBUTE(type, length) __attribute__((vector_size(sizeof(type) * (length))))
...
typedef double vdouble VATTRIBUTE(double, VECTLENDP);
typedef int32_t vint VATTRIBUTE(int32_t, VECTLENDP);
typedef int64_t vint64 VATTRIBUTE(int64_t, VECTLENDP);

typedef float vfloat VATTRIBUTE(float, VECTLENSP);
typedef int32_t vint2 VATTRIBUTE(int32_t, VECTLENSP);

#ifdef ENABLE_LONGDOUBLE
typedef uint8_t vmaskl VATTRIBUTE(long double, VECTLENDP);
typedef long double vlongdouble VATTRIBUTE(long double, VECTLENDP);
#endif

#if defined(Sleef_quad2_DEFINED) && defined(ENABLEFLOAT128)
typedef uint8_t vmaskq VATTRIBUTE(long double, VECTLENDP);
typedef Sleef_quad vquad VATTRIBUTE(long double, VECTLENDP);
#endif
#endif
...
static INLINE vdouble vrev21_vd_vd(vdouble vd) { return __builtin_shuffle(vd, (vint64) {1, 0}); }//(vdouble) { vd[1], vd[0] }; }
static INLINE vdouble vreva2_vd_vd(vdouble vd) { return vd; }
static INLINE vfloat vrev21_vf_vf(vfloat vd) { return __builtin_shuffle(vd, (vint2) {1, 0, 3, 2}); }//(vfloat) { vd[1], vd[0], vd[3], vd[2] }; }
static INLINE vfloat vreva2_vf_vf(vfloat vd) { return __builtin_shuffle(vd, (vint2) {2, 3, 0, 1}); }//(vfloat) { vd[2], vd[3], vd[0], vd[1] }; }
#ifdef ENABLE_LONGDOUBLE
...
static INLINE vdouble vrev21_vd_vd(vdouble vd) { return __builtin_shuffle(vd, (vint64) {1, 0, 3, 2}); }//(vdouble) { vd[1], vd[0], vd[3], vd[2] }; }
static INLINE vdouble vreva2_vd_vd(vdouble vd) { return __builtin_shuffle(vd, (vint64) {2, 3, 0, 1}); }//(vdouble) { vd[2], vd[3], vd[0], vd[1] }; }
static INLINE vfloat vrev21_vf_vf(vfloat vd) { return __builtin_shuffle(vd, (vint2) {1, 0, 3, 2, 5, 4, 7, 6}); }//(vfloat) { vd[1], vd[0], vd[3], vd[2], vd[5], vd[4], vd[7], vd[6] }; }
static INLINE vfloat vreva2_vf_vf(vfloat vd) { return __builtin_shuffle(vd, (vint2) {6, 7, 4, 5, 2, 3, 0, 1}); }//(vfloat) { vd[6], vd[7], vd[4], vd[5], vd[2], vd[3], vd[0], vd[1] }; }
#ifdef ENABLE_LONGDOUBLE
...
//
// b for builtin
static INLINE vdouble vbcopysign_vd_vd_vd(vdouble x, vdouble y) { return (vdouble) kvx_copysign(x, y); }

static INLINE vdouble vsel_vd_vo_vd_vd(vopmask o, vdouble x, vdouble y) { return (vdouble) kvx_select(x, y, o, ".nez"); }
static INLINE vint2 vsel_vi2_vo_vi2_vi2(vopmask o, vint2 x, vint2 y) { return (vint2) kvx_select(x, y, o, ".nez"); }
...
static INLINE vdouble vadd_vd_vd_vd(vdouble x, vdouble y) { return kvx_fadd(x, y, ""); }
static INLINE vdouble vsub_vd_vd_vd(vdouble x, vdouble y) { return x - y; }
static INLINE vdouble vmul_vd_vd_vd(vdouble x, vdouble y) { return kvx_fmul(x, y, ""); }
static INLINE vdouble vdiv_vd_vd_vd(vdouble x, vdouble y) { return x / y; }

static INLINE vdouble vabs_vd_vd(vdouble d) { return (vdouble) kvx_fabs(d); }
static INLINE vdouble vneg_vd_vd(vdouble d) { return kvx_fneg(d); }
static INLINE vdouble vmax_vd_vd_vd(vdouble x, vdouble y) { return kvx_fmax(x, y); }
static INLINE vdouble vmin_vd_vd_vd(vdouble x, vdouble y) { return kvx_fmin(x, y); }
...
static INLINE vdouble vmla_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return kvx_ffma(x, y, z, ""); }
static INLINE vdouble vmlapn_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return -kvx_ffms(x, y, z, ""); }
static INLINE vdouble vmlanp_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return kvx_ffms(x, y, z, ""); }

//

// FMA

static INLINE vdouble vfma_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return kvx_ffma(x, y, z, ""); }
static INLINE vdouble vfmapp_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return kvx_ffma(x, y, z, ""); }
static INLINE vdouble vfmapn_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return -kvx_ffms(x, y, z, ""); }
static INLINE vdouble vfmanp_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return kvx_ffms(x, y, z, ""); }
static INLINE vdouble vfmann_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return -kvx_ffms(-x, y, z, ""); }
...
static INLINE vint vsel_vi_vo_vi_vi(vopmask m, vint x, vint y) {
  return (vint) kvx_select(x, y, vreinterpretFirstHalf_vi_vi2((vint2)m), ".nez");
}
...
static INLINE vdouble vrec_vd_vd(vdouble x) { 
  vdouble vd;
  vd = kvx_fsrec(x, "");
  vd = vfma_vd_vd_vd_vd(vfma_vd_vd_vd_vd(-x, vd, vcast_vd_d(1.0)), vd, vd);
  vd = vfma_vd_vd_vd_vd(vfma_vd_vd_vd_vd(-x, vd, vcast_vd_d(1.0)), vd, vd);
  vd = vfma_vd_vd_vd_vd(vfma_vd_vd_vd_vd(-x, vd, vcast_vd_d(1.0)), vd, vd);
  
  vd = vsel_vd_vo_vd_vd(veq_vo_vd_vd(x, vcast_vd_d(1.7976931348623157081e+308)), vcast_vd_d(+0.0), vd);
  vd = vsel_vd_vo_vd_vd(veq_vo_vd_vd(x, vcast_vd_d(1.000000000000000011e+308)), vcast_vd_d(9.9999999999999990933e-309), vd); // 1/1.0e+308
  vd = vsel_vd_vo_vd_vd(visinf_vo_vd(x), vbcopysign_vd_vd_vd(vcast_vd_d(0.0), x), vd);
  return vd;
...
static INLINE vdouble vsqrt_vd_vd(vdouble d) {
  ...
  /*vdouble x = kvx_fsrsr(d, "");
  x = vfma_vd_vd_vd_vd(vfma_vd_vd_vd_vd(-d, x, vcast_vd_d(1.0)), x, x);
  x = vfma_vd_vd_vd_vd(vfma_vd_vd_vd_vd(-d, x, vcast_vd_d(1.0)), x, x);
  x = vfma_vd_vd_vd_vd(vfma_vd_vd_vd_vd(-d, x, vcast_vd_d(1.0)), x, x);
  x = vrec_vd_vd(x);*/
...
static INLINE void vscatter2_v_p_i_i_vd(double *ptr, int offset, int step, vdouble v) {
  for(int i=0;i<VECTLENDP/2;i++) {
    *(ptr+(offset + step * 0)*2 + 0) = v[0*2+0];
    *(ptr+(offset + step * 0)*2 + 1) = v[0*2+1];
#if VECTLENDP == 4
    *(ptr+(offset + step * 1)*2 + 0) = v[1*2+0];
    *(ptr+(offset + step * 1)*2 + 1) = v[1*2+1];
#endif
  }
}
...
static INLINE vfloat vsel_vf_vo_vf_vf(vopmask o, vfloat x, vfloat y) { return kvx_select(x, y, o, ".nez"); }
...
// b for builtin
static INLINE vfloat vbcopysign_vf_vf_vf(vfloat x, vfloat y) { return kvx_copysign(x, y); }
...
static INLINE vfloat vrec_vf_vf(vfloat x) { return kvx_frec(x, ""); }

static INLINE vfloat vabs_vf_vf(vfloat f) { return kvx_fabs(f); }
...
static INLINE vfloat vmla_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return kvx_ffma(x, y, z, ""); }
static INLINE vfloat vmlapn_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return -kvx_ffms(x, y, z, ""); }
static INLINE vfloat vmlanp_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return kvx_ffms(x, y, z, ""); }

//

// FMA

static INLINE vfloat vfma_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return kvx_ffma(x, y, z, ""); }
static INLINE vfloat vfmapp_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return kvx_ffma(x, y, z, ""); }
static INLINE vfloat vfmapn_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return -kvx_ffms(x, y, z, ""); }
static INLINE vfloat vfmanp_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return kvx_ffms(x, y, z, ""); }
static INLINE vfloat vfmann_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return -kvx_ffms(-x, y, z, ""); }
...
static INLINE void vscatter2_v_p_i_i_vf(float *ptr, int offset, int step, vfloat v) {
    ...
    *(ptr+(offset + step * 1)*2 + 1) = v[1*2+1];
#if VECTLENSP == 8
    *(ptr+(offset + step * 2)*2 + 0) = v[2*2+0];
    *(ptr+(offset + step * 2)*2 + 1) = v[2*2+1];
    *(ptr+(offset + step * 3)*2 + 0) = v[3*2+0];
    *(ptr+(offset + step * 3)*2 + 1) = v[3*2+1];
#endif
}
